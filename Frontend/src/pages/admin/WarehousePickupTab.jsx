...
const WarehousePickupTab = ({ canShip, onActionSuccess }) => {
    ...
    const fetchPickupOrders = async (page = 1, limit = 5) => {
        setLoading(true);
        try {
            ...
            if (res && res.EC === 0) {
                ...
                message.error(res.EM || 'Không tải được danh sách đơn hàng chờ nhận tại quầy');
            }
        } catch (error) {
            console.error('>>> Error fetching pickup orders:', error);
            message.error('Lỗi kết nối khi tải danh sách đơn nhận tại quầy');
        } finally {
            setLoading(false);
        }
    };
    ...
    useEffect(() => {
        fetchPickupOrders(pagination.current, pagination.pageSize);
    }, []);

    const handleTableChange = (newPagination) => {
        fetchPickupOrders(newPagination.current, newPagination.pageSize);
    };
    ...
    const handleConfirmPickup = async (orderId) => {
        try {
            ...
            if (res && res.EC === 0) {
                message.success('Khách hàng đã nhận hàng tại quầy thành công!');
                onActionSuccess();
                fetchPickupOrders(pagination.current, pagination.pageSize);
            } else {
            ...
            console.error('>>> Error completing store pickup order:', error);
            message.error('Lỗi khi cập nhật trạng thái đơn nhận tại quầy');
        }
    };
            ...
            render: (_, record) => {
                const btn = (
                    <Button
                        ...
                        size="small"
                        style={{ backgroundColor: canShip ? '#52c41a' : undefined, borderColor: canShip ? '#52c41a' : undefined }}
                    >
                        Khách đã nhận hàng
                    </Button>
                );
                ...
                if (!canShip) {
                    return (
                        <Tooltip title="Bạn không có quyền xác nhận bàn giao nhận tại quầy (Yêu cầu: orders.update_ship)">
                            <div>{btn}</div>
                        </Tooltip>
                    );
                }
    ...
    return (
        <Table
            ...
            pagination={{
                ...
                showTotal: (total) => `Tổng cộng ${total} đơn chờ nhận tại quầy`
            }}
...
export default WarehousePickupTab;
